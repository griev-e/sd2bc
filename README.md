...
- **Live blip** — a real GPS dot (with its accuracy halo) for where the phone
  ...
  map's locate button), remembered per device, and never shared or stored.
- **Plan marker** — a clock toggle on the map shows where the schedule says you
  should be *right now*: the vehicle emoji sits at that point on the loop and
  creeps along with the clock, with a HUD naming the day and trip progress.
  With the live blip on, the HUD also says how far ahead of or behind schedule
  you're running, measured along the route.
